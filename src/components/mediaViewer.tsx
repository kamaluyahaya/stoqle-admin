...
export default function MediaViewer({
        ...
        {(payload.productVideo?.url || hasStyles) ? (
          ...
          <div className="absolute bottom-4 right-4 z-40 pointer-events-none">
            <div className="px-2.5 py-1.5 bg-black/40 backdrop-blur-md rounded-full border border-white/10 shadow-xl flex items-center gap-1.5 transition-all duration-500 scale-95 group-hover:scale-100">
              <span className="text-[10px] px-2.5 font-black text-white drop-shadow-sm">{selectedIndex + 1}<span className="opacity-40 mx-0.5">/</span>{images.length}</span>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div ref={containerRef} className="w-full flex-1 flex items-center justify-center relative overflow-hidden group bg-slate-100 h-[70vh] max-h-[70vh] lg:h-full lg:max-h-none lg:min-h-0">
      ...
      {!isFullScreen && viewMode !== "video" && !(payload.productVideo?.url || hasStyles) && images.length > 1 && (
        <div className="absolute bottom-4 right-4 z-[45] pointer-events-none">
          <div className=" bg-black/40 px-2.5 py-1 backdrop-blur-md rounded-full shadow-xl flex items-center transition-all duration-500 scale-95 group-hover:scale-100">
...
export function FullscreenImageViewer({
  ...
  const slides = src ? [{ src }] : images.map(img => ({ src: img }));
  ...
  return (
    <Lightbox
      ...
      portal={{ root: typeof document !== "undefined" ? document.body : undefined }}
      controller={{ closeOnBackdropClick: true, closeOnPullDown: true, closeOnPullUp: true }}
      styles={{ root: { zIndex: 9999999 }, container: { backgroundColor: "rgba(0,0,0,0.95)" } }}
    />
  );
}